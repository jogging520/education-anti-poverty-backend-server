...
public class UserService {
     ...
     * @return 新用户
     */
    public Mono<User> createUser(@RequestParam String serialNo,
                                 @RequestParam String appType,
                                 @RequestParam String category,
                                 @RequestBody User user) {
        String salt = Password.generateSalt();

        return this.userRepository.findByCategoryAndStatusAndIdOrName(category,
                Constants.USER_STATUS_ACTIVE, user.getId(), this.crypt.decrypt4UserDownStream(user.getName(), appType, false))
                .map(newUser -> newUser.setStatus(Constants.USER_ERRORCODE_HAS_EXISTS))
                .switchIfEmpty(this.userRepository.save(user.setName(this.crypt.decrypt4UserDownStream(user.getName(), appType, false))
    ...
    public Mono<Authentication> verifyUserByNameAndPassword(String serialNo,
                                                            ...
                                                            String name,
                                                            String password) {
        return this.userRepository.findByCategoryAndStatusAndName(category, Constants.USER_STATUS_ACTIVE, this.crypt.decrypt4UserDownStream(name, appType, true))
                .flatMap(user -> {
                    log.info(Constants.USER_OPERATION_SERIAL_NO + serialNo);
                    ...
                    if (user.getAppTypes() != null &&
                        ...
                        return Mono.just(Authentication.builder()
                                .user(user.getId())
                ...
                Mono.just(Authentication.builder()
                        .status(Constants.USER_AUTHENTICATION_HTTP_REQUEST_MAPPING)
                        .build())
        );
    }
    ...
    public Mono<Authentication> verifyByMobile(String serialNo,
                                               ...
                                               String category,
                                               String mobile) {
        return this.userRepository.findByCategoryAndStatusAndMobilesContaining(category,Constants.USER_STATUS_ACTIVE,
                mobile)
                .flatMap(user -> {
                    log.info(Constants.USER_OPERATION_SERIAL_NO + serialNo);
                    ...
                    if(user.getType() != null &&
                        ...
                        return Mono.just(Authentication.builder()
                                         .user(user.getId())
                                        .authType(Constants.USER_LOGGING_TYPE_CAPTCHA).status(Constants.USER_ERRORCODE_SUCCESS)
                                        .build()
                        );
                    }
                    return Mono.just(Authentication.builder()
                    .status(Constants.USER_ERRORCODE_AUTHENTICATION_FAILURE).build());
                }).switchIfEmpty(Mono.just(Authentication.builder()
                        .status(Constants.USER_ERRORCODE_AUTHENTICATION_FAILURE).build()));
    }
